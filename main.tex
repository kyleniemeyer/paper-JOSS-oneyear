...
In its first year, starting in May 2016, \joss{} published 111 articles, with more than 40 additional articles \delete{currently} under review.
...
\add{By its first anniversary, \joss{} had published more than a hundred articles.}
...
(Since then, we \delete{have} added three more editors: Jason Clark, Lindsey Heagy, and Thomas Leeper.)
...
\begin{itemize}
    ...
    \item The software should be a significant new contribution to the available open-source software that either enables some new research challenge(s) to be addressed or makes addressing research challenges significantly better (e.g., faster, easier, \add{simpler).}
...
\joss{} conducts reviews on the \texttt{joss-reviews} GitHub repository~\cite{joss-reviews}.  Review of a submission begins \delete{by}\add{with} the opening of a new GitHub issue, where the editor-in-chief assigns an editor, the editor assigns a reviewer, and interactions between authors, reviewer(s), and editor proceed in the open. Figure~\ref{fig:review} shows an example of a recent review for the (accepted) \texttt{hdbscan} package~\cite{McInnes2017}.
...
Like a true conversation, discussion can go back and forth in minutes or seconds, with all parties contributing at will. This contrasts \add{with} traditional journal reviews, where the process is merely an exchange between the reviewer(s) and author(s), via the editor, which can take months for each communication, and in practice is limited to one or two, perhaps three in some cases, exchanges due to that delay~\cite{tennant-peerreview}.
...
\begin{itemize}
    \item \textbf{Repository}: Is the source code for this software available at the repository \add{URL}?
...
\begin{itemize}
    ...
    \item \textbf{Community guidelines}: Are there clear guidelines for third parties wishing to 1) contribute to the software\add{,} 2) report issues or problems with the software, and 3) seek support?
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The second year for \joss{}}

Our focus for the \add{second} year will be on continuing to provide a high-quality experience
...
In the \add{second} year, we plan to complete a number of high-priority improvements to the \joss{} toolchain.
...
% Following semantic versioning practices\footnote{Semantic versioning refers to the practice where software versions are of the form \texttt{MAJOR.MINOR.PATCH}, see \href{http://semver.org}{semver.org}).}, a small
...
services such as \add{Figshare}, Zenodo, Dryad, arXiv, or engrXiv\slash AgriXiv\slash LawArXiv\slash PsyArXiv\slash SocArXiv\slash bioRxiv.
